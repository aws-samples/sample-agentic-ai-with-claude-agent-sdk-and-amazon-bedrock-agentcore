...
def build_agent_options(
    ...
    **overrides: Any,
    ...
    # Observability埋点: register the PreToolUse/PostToolUse hooks that emit per-tool
    # OTEL spans + I/O-summary logs for AgentCore Evaluation. hooks_config() returns
    # None (a no-op) unless AGENT_OBSERVABILITY_ENABLED is set — i.e. only inside the
    # deployed runtime — so local run_query() and the fast tests are unaffected.
    import observability

    _hooks = observability.hooks_config()
