...
# OTEL instrumentation for AgentCore Evaluation (no-op unless AGENT_OBSERVABILITY_ENABLED).
...
async def invoke(payload: dict, context=None):
    ...
    The 2nd param MUST be named ``context`` for the runtime to deliver the request
    context (→ ``context.session_id``, used to tie all of a session's spans together
    for the session-level GoalSuccessRate evaluator).
    ...
    async with ClaudeSDKClient(options=options) as client:
        # run_instrumented emits the Strands-compatible span tree + I/O-summary logs that
        # AgentCore Evaluation consumes, and yields the same text chunks. When
        # observability is disabled it falls back to a plain stream (identical behavior).
